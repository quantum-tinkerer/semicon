...
Tx = 1/(3*sympy.sqrt(2)) * sympy.Matrix([[-sympy.sqrt(3),0,1,0], [0,-1,0,sympy.sqrt(3)]])
Ty = -sympy.I/(3*sympy.sqrt(2)) * sympy.Matrix([[sympy.sqrt(3),0,1,0], [0,1,0,sympy.sqrt(3)]])
Tz = sympy.sqrt(2)/3 * sympy.Matrix([[0,1,0,0],[0,0,1,0]])

Txx = 1 / (3*sympy.sqrt(2)) * sympy.Matrix([[0,-1,0,sympy.sqrt(3)],[-sympy.sqrt(3),0,1,0]])
Tyy = 1 / (3*sympy.sqrt(2)) * sympy.Matrix([[0,-1,0,-sympy.sqrt(3)],[sympy.sqrt(3),0,1,0]])
Tzz = sympy.sqrt(2) / 3       * sympy.Matrix([[0,1,0,0],[0,0,-1,0]])

Tyz =  sympy.I /(2*sympy.sqrt(6)) * sympy.Matrix([[-1,0,-sympy.sqrt(3),0],[0,sympy.sqrt(3),0,1]])
Tzx = 1 / (2*sympy.sqrt(6)) * sympy.Matrix([[-1,0,sympy.sqrt(3),0],[0,sympy.sqrt(3),0,-1]])
...
Jx = sympy.Rational(1,2) * \
     sympy.Matrix([[0, sympy.sqrt(3), 0, 0], [sympy.sqrt(3), 0, 2, 0],
                   [0, 2, 0, sympy.sqrt(3)], [0, 0, sympy.sqrt(3), 0]])

Jy = sympy.I * sympy.Rational(1,2) * \
     sympy.Matrix([[0, -sympy.sqrt(3), 0, 0], [sympy.sqrt(3), 0, -2, 0],
                   [0, 2, 0, -sympy.sqrt(3)], [0, 0, sympy.sqrt(3), 0]])
...
B_6c6c = sympy.Rational(1,2) * g * u_b * (sigma_x * Bx + sigma_y * By + sigma_z * Bz)
B_8v8v = -2 * u_b * (kappa * (Jx * Bx + Jy * By + Jz * Bz) + q * (Jx**3 * Bx + Jy**3 * By + Jz**3 * Bz))
B_7v8v = -3 * u_b * kappa * (Tx * Bx + Ty * By + Tz * Bz)
B_8v7v = -3 * u_b * kappa * (Tx * Bx + Ty * By + Tz * Bz).conjugate().transpose()
